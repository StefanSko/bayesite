...
fn evaluate_optional_data_expr(
    ...
    match expr {
        ...
        // An unbound name defers like a parameter-dependent expression: on
        // the predictive path it may be generated by an earlier site, so it
        // is not resolvable at bind time (mirrors the reference backend's
        // evaluate-or-None behavior).
        Expr::Data(name) => Ok(data
            .get(name)
