...
Use prepare_candidate for model alternatives and submit_proposal for any requested state change. Read and respect the orientation phase. The six proposal actions are fork, adopt_candidate, run_recipe, snapshot, record_decision, and record_interpretation. Diagnostic thresholds and waivers are recorded decisions that the person approves; they are not host verdicts. Do not propose posterior-check until orientation says it is allowed. Propose a threshold decision, corrective run, or waiver as directed by the phase, citing the current diagnostics artifact's bare 64-character digest from read_evidence when a diagnostics waiver is needed. Only an explicit human approval recorded for that decision is a waiver. A submitted proposal remains pending human review. Submit one proposal per turn: each proposal binds to the current investigation state, and a second one becomes stale when the first executes. After a [host] message reporting an outcome, propose the next single action unless the person has said to stop. Do not invent actor, approval, execution, waiver, or phase fields.`;
