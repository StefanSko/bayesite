...
the same decision fields as candidate adoption. Diagnostic thresholds are not
hard-coded verdicts: a person approves a `thresholds:` decision containing any
nonempty subset of `rhat<=NUMBER`, `ess>=NUMBER`, and
`divergences<=NUMBER`. The host applies those values mechanically. An exceeded
threshold blocks posterior checking until a diagnostics-citing `waiver:`
decision has an explicitly recorded human-approval child; a recommendation
...
**Amended after review (orientation follow-up).** Every orientation includes
`allowed_actions`, generated by the same phase rules the host enforces, and
`phase_facts`. The latter records the simulation-format waiver status,
historical evidence names, newest approved threshold-decision id, and whether
the estimand is a current free slot. Recipe permissions use
`run_recipe:<operation>`. Missing thresholds produce a `record-thresholds` next
step with the current diagnostics and conventional 1.01/400/0 starting values.
The `simulation-unsupported` next step appears in sampling and checking phases
until an approved `waiver:` decision exists; it is informational and never
blocks execution.

**Amended after review (gate follow-up).** Restart durability tests now compare
complete host records and raw authority-file bytes across independent hosts,
deleted sessions, and a newly created faux-provider session. Duplicate review
and execution coverage also verifies unchanged review metadata and exactly one
attempt directory entry.
