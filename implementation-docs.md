...
Rounds 3-9 (one finding per round; final verdict round 10: **clean** —
"Didn't find any major issues"):

- **iid-scalar restriction on restricted draws** — declined: the reference
  raises the identical error; extending the supported surface is
  upstream-first. Documented as a code comment.
- **recover/SBC truth for partially-observed models** — real gap, re-raised
  as P1. Resolution: fail-fast precondition in `generated_fit` (reject before
  the prior draw with the existing contract error) per the finding's own
  alternative; full support tracked as #28 (needs contract-text versioning
  and possibly bayescycle scenario-schema coordination).
- **dynamic (parameter-dependent) Uniform support not folded** — declined:
  reference's `_evaluate_optional_data_expr` returns None identically; both
  engines rely on -inf rejection there.
- **non-VectorBounds constraints on scatter free slots silently dropped in
  forward draws** — fixed: explicit unsupported error (the reference silently
  mis-samples this corner; no-silent-wrong-answers wins over parity when
  parity means reproducing a reference bug that only affects forward draws,
  not density).
- **free-value shape vs missing_idx length unchecked in forward draws** —
  fixed: rejected before drawing, mirroring the density path's scatter check.
- **support folding errored on DataRefs generated by earlier sites** —
  fixed: unbound names defer like parameter-dependent expressions
  (reference's evaluate-or-None); restricted Uniform draws still respect base
  support at draw time.
- **first-match site selection for support edges is ambiguous** — declined
  for parity (both engines pick the same site in document order, so densities
  agree even on ambiguous documents); the spec question filed upstream as
  bayescycle#56.

Pattern worth keeping: the parity test for accept/decline was "would the
change make the two engines disagree on a document they both accept?" —
numerical robustness inside one engine's stochastic paths: fix freely;
supported-surface or density semantics: upstream first.
